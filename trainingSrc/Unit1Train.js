            ...
            for (let activationFunc = 0; activationFunc < activationFuncs.length; activationFunc++) {
                async function run() {
                    await new Promise((resolve) => {
                        ...
                        fs.appendFileSync('./unit1Report.txt', nodes[node] + ',[' + units[unit][0] + ']\n', 'utf8');

                        let start = new Date();
                        model.fit(trainDataTensor, trainLabelTensor, fitParam).then(async function (result) {
                            fs.appendFileSync('./unit1Report.txt', history[history.length - 1].loss + ',', 'utf8');
                            ...
                            for (let checkPoint = 0; checkPoint < checkPoints.length; checkPoint++) {
                                ...
                                fs.appendFileSync('./unit1Report.txt', good + ',' + noGood + ',', 'utf8');
                            ...
                            model.save(`file://../models/${nodes[node]}_[${units[unit]}]`).then(async function () {
                                ...
                                let finish = new Date();
                                fs.appendFileSync('./unit1Report.txt', finish - start + '\n', 'utf8');
                                resolve();
                            });
                        });
                    });
