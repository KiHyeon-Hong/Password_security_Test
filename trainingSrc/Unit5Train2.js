                ...
                async function run() {
                    await new Promise((resolve) => {
                        ...
                        model.fit(trainDataTensor, trainLabelTensor, fitParam).then(async function (result) {
                            fs.appendFileSync('./unit5Report2.txt', history[history.length - 1].loss + ',', 'utf8');
                            ...
                            for (let checkPoint = 0; checkPoint < checkPoints.length; checkPoint++) {
                                ...
                                fs.appendFileSync('./unit5Report2.txt', good + ',' + noGood + ',', 'utf8');
                            ...
                            model.save(`file://../models/${nodes[node]}_[${units[unit]}]`).then(async function () {
                                ...
                                let finish = new Date();
                                fs.appendFileSync('./unit5Report2.txt', finish - start + '\n', 'utf8');
                                resolve();
                            });
                        });
                    });
