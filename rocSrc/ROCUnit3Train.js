    ...
    for (let i = 1; i <= 32; i = i * 2) {
        for (let j = 1; j <= 32; j = j * 2) {
            for (let k = 1; k <= 32; k = k * 2) {
                fs.writeFileSync(__dirname + `/../ROCData/3_[${i},${j},${k}].txt`, '', 'utf8');

                for (let d = 0; d < testData.length; d++) {
                    const loadedModel = await tf.loadLayersModel('file://' + __dirname + `/../models/3_[${i},${j},${k}]/model.json`);
                    var predictPoint = loadedModel.predict(tf.tensor([testData[d]]));
                    predictPoint = Array.from(predictPoint.dataSync())[0];
                    fs.appendFileSync(__dirname + `/../ROCData/3_[${i},${j},${k}].txt`, `${predictPoint},${testLabel[d]}`, 'utf8');

                    if (d != testData.length - 1) fs.appendFileSync(__dirname + `/../ROCData/3_[${i},${j},${k}].txt`, `\n`, 'utf8');
                }
            }
        }
    }
