...
for (let i = 64865; i < leakString.length - 1; i++) {
    ...
    testString.push(leakString[i]);
}

fs.writeFileSync(__dirname + `/../files/origPasswordSecurity.csv`, `password,zxcvbn,luds,levenshtein,label\n`, `utf8`);

testData.map((data, index) => {
    fs.appendFileSync(__dirname + `/../files/origPasswordSecurity.csv`, `${testString[index]},${data[0]},${data[1]},${data[2]},${testLabel[index]}\n`, `utf8`);
});
