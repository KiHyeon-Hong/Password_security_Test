...
async function passwordValidation() {
    for (let i = 1; i <= 32; i = i * 2) {
        for (let j = 1; j <= 32; j = j * 2) {
            fs.writeFileSync(__dirname + `/../ROCData/2_[${i},${j}].txt`, '', 'utf8');

            for (let d = 0; d < testData.length; d++) {
                const loadedModel = await tf.loadLayersModel('file://' + __dirname + `/../models/2_[${i},${j}]/model.json`);
                var predictPoint = loadedModel.predict(tf.tensor([testData[d]]));
                predictPoint = Array.from(predictPoint.dataSync())[0];
                fs.appendFileSync(__dirname + `/../ROCData/2_[${i},${j}].txt`, `${predictPoint},${testLabel[d]}`, 'utf8');

                if (d != testData.length - 1) fs.appendFileSync(__dirname + `/../ROCData/2_[${i},${j}].txt`, `\n`, 'utf8');
            }
        }
    }
}
