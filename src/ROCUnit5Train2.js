        ...
        for (let j = 1; j <= 32; j = j * 2) {
            for (let k = 1; k <= 32; k = k * 2) {
                for (let l = 1; l <= 32; l = l * 2) {
                    fs.writeFileSync(__dirname + `/../ROCData/5_[2,${i},${j},${k},${l}].txt`, '', 'utf8');

                    for (let d = 0; d < testData.length; d++) {
                        const loadedModel = await tf.loadLayersModel('file://' + __dirname + `/../models/5_[2,${i},${j},${k},${l}]/model.json`);
                        var predictPoint = loadedModel.predict(tf.tensor([testData[d]]));
                        predictPoint = Array.from(predictPoint.dataSync())[0];
                        fs.appendFileSync(__dirname + `/../ROCData/5_[2,${i},${j},${k},${l}].txt`, `${predictPoint},${testLabel[d]}`, 'utf8');

                        if (d != testData.length - 1) fs.appendFileSync(__dirname + `/../ROCData/5_[2,${i},${j},${k},${l}].txt`, `\n`, 'utf8');
                    }
                }
            }
        }
