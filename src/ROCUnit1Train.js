...
async function passwordValidation() {
    for (let i = 1; i <= 32; i = i * 2) {
        fs.writeFileSync(__dirname + `/../ROCData/1_[${i}].txt`, '', 'utf8');

        for (let d = 0; d < testData.length; d++) {
            const loadedModel = await tf.loadLayersModel('file://' + __dirname + `/../models/1_[${i}]/model.json`);
            var predictPoint = loadedModel.predict(tf.tensor([testData[d]]));
            predictPoint = Array.from(predictPoint.dataSync())[0];
            fs.appendFileSync(__dirname + `/../ROCData/1_[${i}].txt`, `${predictPoint},${testLabel[d]}`, 'utf8');

            if (d != testData.length - 1) fs.appendFileSync(__dirname + `/../ROCData/1_[${i}].txt`, `\n`, 'utf8');
        }
    }
}
